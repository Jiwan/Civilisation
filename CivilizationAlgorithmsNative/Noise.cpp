...
void initBruit2D(int l, int h, int p, int n) {
    ...
    hauteur = h;
    pas2D = p;
    int longueur_max = static_cast<int> ceil(longueur * pow(2, nombre_octaves2D  - 1)  / pas2D);
    int hauteur_max = static_cast<int> ceil(hauteur * pow(2, nombre_octaves2D  - 1)  / pas2D);
